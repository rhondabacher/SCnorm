...
<<style-knitr, eval=TRUE, echo=FALSE, results=tex, cahce=TRUE>>=
  BiocStyle::latex()
@
...
If SavePD=TRUE is specified, the plots of the progress of SCnorm will be created and saved to the directly and file names given to MyNormalizedData for each value of K tried. Setting SavePDF=FALSE will print plots to screen or current device.
