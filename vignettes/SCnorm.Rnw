...
The easiest way to install SCnorm is through Bioconductor(https://bioconductor.org/packages/devel/bioc/html/SCnorm.html):
...
There are three additional ways to download SCnorm.
...
The third option is the to download the Bioconductor version through GitHub. It requires having at least R version 3.4.0 and makes use of SummarizedExperiment classes and BiocParallel.
