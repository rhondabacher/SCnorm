...
There are three ways to download SCnorm (shown below).
...
The third option is the version accepted by BioConductor and requires having at least R version 3.4.0 and makes use of summarizedExperiment 
and BiocParallel.
<<eval=FALSE, echo=TRUE>>=
install.packages("SCnorm_x.x.x.tar.gz", repos=NULL, type="source")
#OR
library(devtools)
devtools::install_github("rhondabacher/SCnorm", ref="develop")
#OR
...
If the input is a \Rclass{SummarizedExperiment} class, then the main assay should contain the expression matrix. 
...
<<eval=TRUE, fig.height=3, fig.width=5, fig.align='center'>>=
...
\large{How can I speedup SCnorm if I have thousands of cells?}
...
\large{SCnorm was unable to converge.}
