...
scaleNormMultCont <- function(NormData, OrigData, Genes, useSpikes) {
    ...
    AllGenes <- Genes
    K <- 4
    avgexp <- log(apply(OrigData[Genes,], 1, function(x) median(x[x!=0])))
    groups <- K
    splitby <- sort(avgexp)
    splitS <- length(splitby)/groups
    sreg <- split(splitby, ceiling(seq_along(splitby) / splitS))

    # Need to put a check here later on to make sure the
    # rownames are in the same order
    OC <- do.call(cbind, lapply(1:NumCond, function(x) {
        cbind(NormData[[x]]$NormData)}))
